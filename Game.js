...
const coinWidth = 30;
const LEVEL_COMPLETION_TIME = 110;
...
function startLevel() {
	...
	coinScoreBoardImg.init(22, 22, 'Pictures/coiny.png', 420, 21, 'image', WALKING);
	...
	var totalEnemies = MAX_VARIABLES_ENEMIES_0;
	if (this.character == 1) {
		totalEnemies = MAX_VARIABLES_ENEMIES_1;
	}
	...
	var totalCoins = MAX_VARIABLES_COINS_0;
	if (this.character == 1) {
		totalCoins = MAX_VARIABLES_COINS_1;
	}
...
function Component() {
	this.init = function (width, height, color, x, y, dataType, moveType, initialShow = false, charName = undefined) {
		...
		if (dataType === 'image') {
			...
			if (charName) {
				...
				this.imageMirror.src = `Pictures/${charName}_left.svg`;
	...
	this.rotation = function () {
		if (coinRotationValue === 0) {
			this.setSrc('Pictures/coiny.png');
		} else if (coinRotationValue === 10) {
			this.setSrc('Pictures/coiny2.png');
		} else if (coinRotationValue === 20) {
			this.setSrc('Pictures/coiny3.png');
		} else if (coinRotationValue === 30) {
			this.setSrc('Pictures/coiny4.png');
		}
	};
...
function gameComplete() {
	...
	var btncharacter1 = document.getElementById('btnfem');
...
function updateGameArea() {
	...
	if (gamePaused) {
		...
		if (musicToggled) { //Then unmute the music, and cancel musicToggled so that this won't re-trigger
			audio.load();
			musicToggled = false; gameCompleteModal
		}
	}
	...
	if (gameArea.time >= LEVEL_COMPLETION_TIME) {
		...
		if (currentLevel === totalLevels) gameComplete();
			...
			levelTransitionModalContent.innerHTML += msg;
